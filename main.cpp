...
void parse(ifstream &myfile, vector<top_table> &cases)
{
    ...
    if (getline(myfile, line))
    {
        ...
        if (regex_search(line, score_table_matches, regex("^(\\d{1})\\s(\\d{1})$")) && cases.size() < cases.capacity())
        {
            ...
            top_table *current_case_p = &cases.back();
            current_case_p->names.reserve(stoi(score_table_matches[2]));
            current_case_p->scores.reserve(stoi(score_table_matches[2]));
        }


        if (regex_search(line, score_table_matches, regex("^(\\w{3})\\s(\\d+)$")))
        {
            top_table *current_case = &cases.back();
            //cout << (current_case->names.size() < current_case->names.capacity()) << endl;
...
int main()
{
    ...
    if (myfile.is_open())
    {
        ...
        cout << "Case 5 name 0 " << cases[2].names.size() << endl;
